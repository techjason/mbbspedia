...
Shock is diagnosed clinically when there is **circulatory failure causing inadequate tissue perfusion**. Hypotension helps, but it is not required early.
...
```mermaid
flowchart TD
    A["Acutely unwell surgical patient"] --> B["ABCDE assessment"]
    B --> C{"Evidence of hypoperfusion?"}
    C -->|"No"| D["Monitor and search for early cause"]
    C -->|"Yes"| E["Shock likely"]

    E --> F["Immediate resuscitation"]
    F --> G["Check skin, JVP, lungs, heart, abdomen, bleeding, temperature"]
    G --> H["Bedside ultrasound or echo"]

    H --> I{"Likely phenotype?"}
    I -->|"Low volume"| J["Hypovolaemic or haemorrhagic"]
    I -->|"Poor pump"| K["Cardiogenic"]
    I -->|"Obstruction"| L["Tamponade, PE, tension pneumothorax"]
    I -->|"Low SVR"| M["Distributive"]
    I -->|"Mixed"| N["Treat dominant immediately reversible cause"]

    J --> O["Control bleeding and replace volume or blood"]
    K --> P["ECG, troponin, echo, inotrope or reperfusion"]
    L --> Q["Immediate decompression or thrombolysis or drainage"]
    M --> R["Source control, antibiotics if septic, vasopressors"]
```
