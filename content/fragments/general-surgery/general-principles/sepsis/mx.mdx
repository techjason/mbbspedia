...
```mermaid
flowchart TD
    A["Suspected sepsis"] --> B["ABCDE + IV access"]
    B --> C["Blood cultures"]
    C --> D["Lactate + organ bloods"]
    D --> E["Empiric antibiotics"]
    E --> F{"Hypoperfusion or shock?"}
    F -->|"Yes"| G["30 mL/kg crystalloid"]
    G --> H["Prefer balanced fluid"]
    H --> I{"MAP < 65?"}
    I -->|"Yes"| J["Norepinephrine"]
    J --> K["Peripheral OK initially"]
    K --> L["Add vasopressin"]
    L --> M["Add epinephrine"]
    ...
    P -->|"Yes"| Q["Control within 6h"]
    P -->|"No"| R["Medical treatment"]
    Q --> S["Reassess perfusion"]
    R --> S
    S --> T["De-escalate antibiotics"]
```
